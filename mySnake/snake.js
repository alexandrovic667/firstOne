...
window.onload = function() {	
	...
	let directions = function(event){
		if(event.keyCode === 37) return "left";
		else if(event.keyCode === 38) return "up";
		else if(event.keyCode === 39) return "right";
		else if(event.keyCode === 40) return "down";
	};
    // Настройка «холста»
	...
	document.addEventListener('keydown', function (event) {
		let newDirection = directions(event);
